# 🎭 Telegram-бот "Отмазочник" v2.0

> **Production-ready бот для генерации креативных отмазок с использованием LLM**

Генерирует отмазки в разных стилях, сохраняет историю, работает с голосовыми сообщениями и поддерживает персонализацию.

**🚀 Попробовать бота: [@kolyanius_fu_bot](https://t.me/kolyanius_fu_bot)**

---

## 📋 Содержание

- [Новые возможности v2.0](#-новые-возможности-v20)
- [Быстрый старт](#-быстрый-старт)
- [Детальная настройка](#-детальная-настройка)
- [Структура проекта](#-структура-проекта)
- [База данных и миграции](#-база-данных-и-миграции)
- [Использование бота](#-использование-бота)
- [API и интеграция](#-api-и-интеграция)
- [Мониторинг и логи](#-мониторинг-и-логи)
- [Troubleshooting](#-troubleshooting)
- [Разработка](#-разработка)

---

## ✨ Новые возможности v2.0

### 🎯 Основные фичи

- **👍👎 Обратная связь** - Оценивай качество отмазок
- **🔄 Регенерация** - Получай альтернативные варианты одним кликом
- **📜 История** - Просматривай последние 10 отмазок
- **⭐ Избранное** - Сохраняй лучшие отмазки
- **🎤 Голосовые сообщения** - Отправляй голосом, получай текстом
- **📊 Статистика** - Отслеживай использование и любимые стили
- **🗄️ PostgreSQL** - Надежное хранение данных
- **🔐 Production-ready** - Полная обработка ошибок и логирование

### 🎨 4 уникальных стиля

- **💪 Быдло** - Грубоватые, прямолинейные отмазки с жаргоном
- **💼 Корпорат** - Деловые, бюрократические отмазки в офисном стиле
- **🙏 Монах** - Философские, мудрые отмазки с духовным подтекстом
- **🔮 Инфоцыган** - Мистические, эзотерические отмазки с магией
- **🎲 Случайный** - Случайный выбор из всех стилей

---

## 🚀 Быстрый старт

### Шаг 1: Клонирование репозитория

```bash
git clone https://github.com/yourusername/kolyanius_fu_bot.git
cd kolyanius_fu_bot
```

### Шаг 2: Настройка окружения

Создайте файл `.env` из шаблона:

```bash
cp env.example .env
```

Отредактируйте `.env` файл:

```bash
# Обязательные параметры
TELEGRAM_BOT_TOKEN=your_bot_token_here
OPENROUTER_API_KEY=your_openrouter_key_here

# Опциональные (можно оставить по умолчанию)
LLM_BASE_URL=https://openrouter.ai/api/v1
DATABASE_URL=postgresql+asyncpg://botuser:botpassword@postgres:5432/otmazochnik

# Whisper API для голосовых сообщений (опционально)
# По умолчанию используется OPENROUTER_API_KEY
# Раскомментируйте если нужен отдельный API ключ для Whisper
# WHISPER_API_KEY=your_openai_key_here
# WHISPER_BASE_URL=https://api.openai.com/v1
```

### Шаг 3: Получение токенов

#### Telegram Bot Token:
1. Откройте Telegram и найдите [@BotFather](https://t.me/BotFather)
2. Отправьте команду `/newbot`
3. Следуйте инструкциям для создания бота
4. Скопируйте полученный токен в `.env` файл

#### OpenRouter API Key (для GPT-4o):
1. Зарегистрируйтесь на [OpenRouter](https://openrouter.ai)
2. Перейдите в раздел API Keys
3. Создайте новый ключ
4. Скопируйте ключ в `.env` файл как `OPENROUTER_API_KEY`

**Альтернатива:** Используйте [gptunnel.ru](https://gptunnel.ru) для доступа к API

#### Whisper API Key (для голосовых сообщений):

**Вариант 1: Использовать тот же ключ (по умолчанию)**
- Если ваш OpenRouter или gptunnel поддерживает Whisper API
- Ничего дополнительно не нужно настраивать
- Бот автоматически использует `OPENROUTER_API_KEY`

**Вариант 2: Отдельный OpenAI ключ**
- Зарегистрируйтесь на [OpenAI](https://platform.openai.com)
- Получите API ключ
- Добавьте в `.env`:
  ```bash
  WHISPER_API_KEY=sk-your_openai_key_here
  WHISPER_BASE_URL=https://api.openai.com/v1
  ```

**Вариант 3: Другой провайдер Whisper API**
- Если у вас свой сервер или другой провайдер
- Укажите соответствующий URL и ключ:
  ```bash
  WHISPER_API_KEY=your_key_here
  WHISPER_BASE_URL=https://your-whisper-api.com/v1
  ```

### Шаг 4: Запуск бота

```bash
# Запуск в Docker (рекомендуется)
docker compose up -d

# Просмотр логов
docker compose logs -f bot

# Остановка
docker compose down
```

**Готово!** Бот запущен и готов к использованию 🎉

---

## 🔧 Детальная настройка

### Переменные окружения

| Переменная | Описание | Обязательно | По умолчанию |
|-----------|----------|-------------|--------------|
| `TELEGRAM_BOT_TOKEN` | Токен Telegram бота | ✅ | - |
| `OPENROUTER_API_KEY` | API ключ для LLM (GPT-4o) | ✅ | - |
| `LLM_BASE_URL` | URL LLM провайдера | ❌ | `https://gptunnel.ru/v1` |
| `WHISPER_API_KEY` | API ключ для Whisper (транскрипция) | ❌ | `OPENROUTER_API_KEY` |
| `WHISPER_BASE_URL` | URL Whisper API | ❌ | `https://api.openai.com/v1` |
| `DATABASE_URL` | URL PostgreSQL базы | ❌ | `postgresql+asyncpg://...` |
| `LOG_LEVEL` | Уровень логирования | ❌ | `INFO` |

### Настройка LLM параметров

Измените в `app/config.py`:

```python
MODEL_NAME: str = "gpt-4o"       # Модель LLM
MAX_TOKENS: int = 500            # Максимум токенов
TEMPERATURE: float = 0.7         # Креативность (0.0 - 1.0)
RETRY_COUNT: int = 1             # Количество повторов
MAX_MESSAGE_LENGTH: int = 200    # Лимит длины сообщений
```

### Настройка Whisper API

#### Как работает Whisper API в боте:

1. **По умолчанию** - использует те же credentials что и LLM:
   - `WHISPER_API_KEY` = `OPENROUTER_API_KEY`
   - `WHISPER_BASE_URL` = `https://api.openai.com/v1`

2. **Отдельная настройка** - укажите в `.env`:
   ```bash
   WHISPER_API_KEY=sk-your_separate_key
   WHISPER_BASE_URL=https://your-provider.com/v1
   ```

3. **Проверка в коде** (app/llm_client.py):
   ```python
   def get_whisper_client():
       """Отдельный клиент для Whisper API"""
       return openai.OpenAI(
           base_url=config.WHISPER_BASE_URL,
           api_key=config.WHISPER_API_KEY,
           timeout=30.0
       )
   ```

#### Поддерживаемые провайдеры Whisper:

- **OpenAI** (рекомендуется): `https://api.openai.com/v1`
- **OpenRouter**: `https://openrouter.ai/api/v1` (если поддерживает Whisper)
- **gptunnel.ru**: `https://gptunnel.ru/v1` (проверьте наличие Whisper)
- **Локальный Whisper**: Ваш собственный endpoint

### Настройка базы данных

#### Изменение паролей PostgreSQL

Отредактируйте `docker-compose.yml`:

```yaml
postgres:
  environment:
    - POSTGRES_DB=otmazochnik
    - POSTGRES_USER=your_custom_user
    - POSTGRES_PASSWORD=your_secure_password
```

Обновите `DATABASE_URL` в `.env`:

```bash
DATABASE_URL=postgresql+asyncpg://your_custom_user:your_secure_password@postgres:5432/otmazochnik
```

#### Подключение к внешней БД

Если у вас уже есть PostgreSQL сервер:

1. Закомментируйте секцию `postgres` в `docker-compose.yml`
2. Измените `DATABASE_URL` в `.env` на ваш сервер
3. Выполните миграции вручную:

```bash
# Установите зависимости
pip install -r requirements.txt

# Выполните миграции
alembic upgrade head
```

---

## 📁 Структура проекта

```
kolyanius_fu_bot/
├── app/                        # Основной код приложения
│   ├── __init__.py            # Package metadata
│   ├── main.py                # Entry point, инициализация
│   ├── config.py              # Конфигурация (включая Whisper)
│   ├── bot.py                 # Telegram handlers (v2.0)
│   ├── llm_client.py          # OpenRouter + Whisper API clients
│   ├── database.py            # Database service layer (NEW)
│   ├── models.py              # SQLAlchemy models (NEW)
│   ├── prompts.py             # LLM промпты
│   └── styles.py              # Определения стилей
│
├── alembic/                   # Database migrations (NEW)
│   ├── versions/              # Файлы миграций
│   ├── env.py                 # Alembic окружение
│   └── script.py.mako         # Шаблон миграций
│
├── logs/                      # Логи (runtime)
│   ├── app.log               # Общие события
│   ├── errors.log            # Ошибки с трейсбэками
│   └── requests.log          # Запросы пользователей
│
├── doc/                       # Документация
│   ├── product_idea.md       # Идея продукта
│   ├── vision.md             # Техническое видение
│   ├── tasklist.md           # План разработки
│   ├── workflow.md           # Процесс разработки
│   └── conventions.md        # Coding conventions
│
├── docker-compose.yml         # Docker Compose config
├── Dockerfile                 # Docker image definition
├── requirements.txt           # Python dependencies
├── alembic.ini               # Alembic configuration (NEW)
├── env.example               # Шаблон .env файла
├── .gitignore                # Git exclusions
├── README.md                 # Этот файл
└── CLAUDE.md                 # Claude Code guidelines
```

---

## 🗄️ База данных и миграции

### Структура базы данных

#### Таблица `users`
```sql
- user_id (BigInteger, PK)          # Telegram User ID
- username (String)                  # @username
- first_name (String)                # Имя пользователя
- created_at (DateTime)              # Дата регистрации
- last_active (DateTime)             # Последняя активность
- default_style (String, nullable)   # Любимый стиль
- is_premium (Boolean)               # Premium статус
```

#### Таблица `excuses`
```sql
- id (Integer, PK, autoincrement)    # ID отмазки
- user_id (BigInteger, FK)           # Ссылка на пользователя
- original_message (Text)            # Исходное сообщение
- style (String)                     # Выбранный стиль
- generated_text (Text)              # Сгенерированная отмазка
- created_at (DateTime, indexed)     # Время создания
- rating (Integer, nullable)         # Оценка: 1 (👍) или -1 (👎)
- response_time (Float, nullable)    # Время генерации (секунды)
```

#### Таблица `favorites`
```sql
- id (Integer, PK, autoincrement)    # ID записи
- user_id (BigInteger, FK)           # Пользователь
- excuse_id (Integer, FK)            # Отмазка
- created_at (DateTime)              # Дата добавления
```

### Работа с миграциями

#### Создание новой миграции

```bash
# Автоматическая генерация миграции
alembic revision --autogenerate -m "Description of changes"

# Ручное создание миграции
alembic revision -m "Description of changes"
```

#### Применение миграций

```bash
# Применить все миграции
alembic upgrade head

# Применить конкретную миграцию
alembic upgrade <revision_id>

# Откатить последнюю миграцию
alembic downgrade -1

# Просмотр истории
alembic history

# Текущая версия
alembic current
```

#### Миграции в Docker

Миграции применяются автоматически при запуске бота через `app/database.py:init_database()`.

Для ручного применения:

```bash
# Войти в контейнер
docker compose exec bot sh

# Применить миграции
alembic upgrade head
```

### Резервное копирование БД

```bash
# Создать backup
docker compose exec postgres pg_dump -U botuser otmazochnik > backup.sql

# Восстановить из backup
docker compose exec -T postgres psql -U botuser otmazochnik < backup.sql
```

---

## 📱 Использование бота

### Основные команды

- `/start` - Запуск бота, регистрация пользователя
- `/help` - Описание всех стилей и инструкции
- `/history` - Просмотр последних 10 отмазок
- `/favorites` - Просмотр избранных отмазок
- `/stats` - Статистика использования

### Рабочий процесс

#### 1. Отправка запроса
- **Текстом:** Просто напишите ситуацию (макс 200 символов)
- **Голосом:** Отправьте голосовое сообщение (автоматическая транскрипция)

#### 2. Выбор стиля
Выберите один из 5 вариантов:
- 💪 Быдло
- 💼 Корпорат
- 🙏 Монах
- 🔮 Инфоцыган
- 🎲 Случайный стиль

#### 3. Работа с результатом
После генерации доступны действия:
- **👍 / 👎** - Оценить качество
- **⭐ В избранное** - Сохранить отмазку
- **🔄 Другой вариант** - Регенерировать в том же стиле

### Примеры использования

#### Пример 1: Текстовый запрос
```
User: не сделал отчёт
Bot: Выбери стиль для отмазки:
     [💪 Быдло] [💼 Корпорат] [🙏 Монах] [🔮 Инфоцыган] [🎲 Случайный]

User: [нажимает 💼 Корпорат]
Bot: Стиль: 💼 Корпорат

     В связи с форс-мажорными обстоятельствами и оптимизацией
     бизнес-процессов в рамках текущего воркфлоу, финализация
     отчётности была перенесена на следующий спринт.

     [👍] [👎] [⭐ В избранное]
     [🔄 Другой вариант]
```

#### Пример 2: Голосовой запрос
```
User: [отправляет голосовое "не успел на встречу"]
Bot: 🎤 Обрабатываю голосовое сообщение...
     Распознано: не успел на встречу

     Выбери стиль для отмазки:
     [💪 Быдло] [💼 Корпорат] [🙏 Монах] [🔮 Инфоцыган] [🎲 Случайный]
```

#### Пример 3: История и избранное
```
User: /history
Bot: 📜 Твоя история (последние 5 отмазок):

     1. 💼 Корпорат 👍
        не сделал отчёт...
        В связи с форс-мажорными...

     2. 💪 Быдло
        опоздал на работу...
        Братан, автобус капец как...
     ...

User: /favorites
Bot: ⭐ Твоё избранное (3 отмазки):

     1. 💼 Корпорат
        не сделал отчёт...
        В связи с форс-мажорными обстоятельствами...
     ...
```

---

## 🔌 API и интеграция

### LLM Provider

Бот использует **OpenRouter** или **gptunnel.ru** для доступа к GPT-4o.

#### Поддерживаемые модели
- `gpt-4o` (по умолчанию, рекомендуется)
- `gpt-4o-mini` (быстрее, дешевле)
- `gpt-3.5-turbo` (бюджетный вариант)

Измените в `app/config.py`:
```python
MODEL_NAME: str = "gpt-4o-mini"  # Для экономии
```

### Voice Transcription (Whisper API)

Голосовые сообщения обрабатываются через **Whisper API**.

#### Архитектура:

```
User Voice Message
    ↓
Telegram Bot API (download .ogg file)
    ↓
get_whisper_client() [app/llm_client.py]
    ↓
WHISPER_BASE_URL + WHISPER_API_KEY
    ↓
Whisper model="whisper-1"
    ↓
Transcribed text
    ↓
Bot generates excuse
```

#### Конфигурация Whisper:

**Файл: app/config.py**
```python
# Whisper API для транскрипции
WHISPER_API_KEY: str = os.getenv(
    "WHISPER_API_KEY",
    os.getenv("OPENROUTER_API_KEY", "")  # fallback на LLM ключ
)
WHISPER_BASE_URL: str = os.getenv(
    "WHISPER_BASE_URL",
    "https://api.openai.com/v1"  # стандартный OpenAI endpoint
)
```

**Файл: app/llm_client.py**
```python
def get_whisper_client():
    """Отдельный клиент для Whisper API"""
    return openai.OpenAI(
        base_url=config.WHISPER_BASE_URL,
        api_key=config.WHISPER_API_KEY,
        timeout=30.0,  # Whisper может быть медленнее
        max_retries=1
    )
```

#### Использование в коде:

**Файл: app/bot.py**
```python
from app.llm_client import get_whisper_client

whisper_client = get_whisper_client()

transcription = await asyncio.get_event_loop().run_in_executor(
    None,
    lambda: whisper_client.audio.transcriptions.create(
        model="whisper-1",
        file=voice_bytes
    )
)
```

#### Требования:
- **Формат:** OGG, MP3, WAV, M4A, WEBM
- **Максимальная длина:** 10 минут (ограничение Whisper API)
- **Размер файла:** До 25 MB
- **Язык:** Автоопределение (поддержка русского)
- **Модель:** `whisper-1` (стандартная модель OpenAI)

#### Проверка работы Whisper:

```bash
# Проверить логи при отправке голосового
docker compose logs -f bot | grep Whisper

# Ожидаемый вывод:
# Initializing Whisper client: https://api.openai.com/v1
# Whisper API key length: 51
# Transcribed voice from user 123456789: не успел на встречу
```

### Database Access

Прямой доступ к БД:

```python
from app import database as db

# Получить историю пользователя
excuses = await db.get_user_history(user_id=123456789, limit=10)

# Добавить в избранное
await db.add_to_favorites(user_id=123456789, excuse_id=42)

# Получить статистику
stats = await db.get_user_stats(user_id=123456789)
```

---

## 📊 Мониторинг и логи

### Структура логирования

Бот использует трехуровневую систему логов:

#### 1. `logs/app.log` - Общие события
```
2025-01-08 12:00:00 - app - INFO - 🚀 Запуск бота 'Отмазочник'
2025-01-08 12:00:01 - app - INFO - 🗄️ Инициализация базы данных...
2025-01-08 12:00:02 - app - INFO - Initializing LLM client: https://gptunnel.ru/v1
2025-01-08 12:00:03 - app - INFO - Initializing Whisper client: https://api.openai.com/v1
2025-01-08 12:00:04 - app - INFO - ✅ База данных готова
2025-01-08 12:00:05 - app - INFO - 🤖 Telegram бот запускается...
```

#### 2. `logs/errors.log` - Ошибки с трейсбэками
```
2025-01-08 12:15:30 - error - ERROR - API_ERROR | User: 123456789 | Style: быдло | Error: Timeout
Traceback (most recent call last):
  ...
```

#### 3. `logs/requests.log` - Запросы пользователей
```
2025-01-08 12:10:00 - requests - INFO - MESSAGE | User: 123456789 (@username) | Text: 'не сделал отчёт' | Length: 15
2025-01-08 12:10:01 - requests - INFO - VOICE | User: 123456789 (@username) | Text: 'не успел на встречу' | Length: 20
2025-01-08 12:10:05 - requests - INFO - STYLE_SELECTED | User: 123456789 (@username) | Selected: корпорат | Actual: корпорат
2025-01-08 12:10:08 - requests - INFO - SUCCESS | User: 123456789 | Style: корпорат | Time: 2.34s | Length: 145 | Attempts: 1
```

### Просмотр логов

```bash
# В реальном времени (Docker)
docker compose logs -f bot

# Конкретный файл
tail -f logs/app.log
tail -f logs/errors.log
tail -f logs/requests.log

# Поиск ошибок
grep ERROR logs/errors.log

# Статистика по пользователю
grep "User: 123456789" logs/requests.log
```

### Мониторинг производительности

```bash
# Статистика времени генерации
grep "SUCCESS" logs/requests.log | awk '{print $14}' | sort -n

# Количество запросов по стилям
grep "STYLE_SELECTED" logs/requests.log | awk '{print $13}' | sort | uniq -c

# Ошибки за последний час
tail -n 1000 logs/errors.log | grep "$(date '+%Y-%m-%d %H')"
```

---

## 🐛 Troubleshooting

### Проблема: Бот не запускается

**Симптомы:**
```
Error: TELEGRAM_BOT_TOKEN is required
```

**Решение:**
1. Проверьте `.env` файл
2. Убедитесь что токен скопирован полностью
3. Перезапустите: `docker compose down && docker compose up -d`

---

### Проблема: Ошибка подключения к БД

**Симптомы:**
```
Error connecting to database
```

**Решение:**
```bash
# Проверить статус PostgreSQL
docker compose ps

# Проверить логи БД
docker compose logs postgres

# Пересоздать БД
docker compose down -v
docker compose up -d
```

---

### Проблема: LLM API не отвечает

**Симптомы:**
```
LLM request timeout
Rate limit exceeded
```

**Решение:**
1. Проверьте баланс на OpenRouter
2. Проверьте доступность `LLM_BASE_URL`
3. Увеличьте timeout в `app/llm_client.py`:
```python
timeout=30.0  # было 15.0
```

---

### Проблема: Голосовые сообщения не работают

**Симптомы:**
```
Error in voice_handler
Error: Invalid API key
Error: Whisper model not found
```

**Решение:**

**Случай 1: Неправильный API ключ**
```bash
# Проверьте логи
docker compose logs bot | grep Whisper

# Ожидаемый вывод:
# Initializing Whisper client: https://api.openai.com/v1
# Whisper API key length: 51  # Должен быть > 0

# Если длина 0 - проверьте .env файл
# Добавьте WHISPER_API_KEY или убедитесь что OPENROUTER_API_KEY валиден
```

**Случай 2: Неверный WHISPER_BASE_URL**
```bash
# Если используете OpenRouter - проверьте поддерживает ли он Whisper
# Если нет - используйте прямой OpenAI endpoint:
WHISPER_API_KEY=sk-your_openai_key
WHISPER_BASE_URL=https://api.openai.com/v1
```

**Случай 3: Модель недоступна**
```bash
# Убедитесь что у вас есть доступ к Whisper API
# OpenAI требует валидный API ключ с активной подпиской
```

**Случай 4: Проверить размер файла**
```bash
# Whisper API ограничен 25 MB
# Проверить в логах размер голосового файла
```

---

### Проблема: Миграции не применяются

**Симптомы:**
```
Target database is not up to date
```

**Решение:**
```bash
# Войти в контейнер
docker compose exec bot sh

# Проверить текущую версию
alembic current

# Применить миграции вручную
alembic upgrade head

# Если не помогло - сбросить БД
docker compose down -v
docker compose up -d
```

---

## 👨‍💻 Разработка

### Локальный запуск без Docker

```bash
# Установить PostgreSQL локально
brew install postgresql  # macOS
sudo apt install postgresql  # Ubuntu

# Создать БД
createdb otmazochnik

# Установить зависимости
pip install -r requirements.txt

# Применить миграции
alembic upgrade head

# Запустить бота
python -m app.main
```

### Тестирование LLM

```bash
# Тест LLM клиента
python test_llm.py

# Ожидаемый вывод:
# ✅ Environment variables loaded
# ✅ OpenAI client created
# ✅ Sync request successful
# ✅ Async request successful
```

### Создание новых стилей

1. Добавьте стиль в `app/styles.py`:
```python
STYLES = {
    "новый_стиль": {
        "name": "Новый стиль",
        "emoji": "🎨",
        "description": "Описание стиля"
    }
}
```

2. Добавьте промпт в `app/prompts.py`:
```python
EXCUSE_PROMPTS = {
    "новый_стиль": """Промпт для нового стиля..."""
}
```

3. Обновите кнопки в `app/bot.py:create_style_keyboard()`

### Deployment в production

#### На VPS с Docker

```bash
# На сервере
git clone https://github.com/yourusername/kolyanius_fu_bot.git
cd kolyanius_fu_bot
cp env.example .env
nano .env  # Настройте переменные

# Запуск
docker compose up -d

# Auto-restart при загрузке сервера
sudo systemctl enable docker
```

#### На облачных платформах

- **Railway.app**: Автодеплой из GitHub
- **Render.com**: Docker Compose support
- **DigitalOcean App Platform**: Managed PostgreSQL

---

## 📄 Лицензия

Проект создан для образовательных целей.

---

## 🤝 Поддержка

- **Issues:** [GitHub Issues](https://github.com/yourusername/kolyanius_fu_bot/issues)
- **Документация:** `doc/` папка
- **Email:** your.email@example.com

---

## 🎯 Roadmap

- [ ] Веб-интерфейс (Telegram Mini App)
- [ ] Мультиязычность (EN, UA)
- [ ] Premium подписка
- [ ] Статистика и аналитика
- [ ] A/B тестирование промптов
- [ ] Экспорт истории в PDF
- [ ] Интеграция с другими мессенджерами

---

**🎭 Бот готов генерировать креативные отмазки в production-режиме!**

_Made with ❤️ using Python, aiogram, PostgreSQL, and GPT-4o + Whisper_
